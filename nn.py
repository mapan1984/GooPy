...
class SearchNet:
    ...
    def setup_netword(self, wordids, urlids):
        # 值列表
    ...
    def get_result(self, wordids, urlids):
        self.setup_netword(wordids, urlids)
    ...
    def update_db(self):
        ...
        for i in range(len(self.wordids)):
            for j in range(len(self.hiddenids)):
                self.set_strength(self.wordids[i], self.hiddenids[j],
                                    0, self.wi[i][j])
        for j in range(len(self.hiddenids)):
            for k in range(len(self.urlids)):
                self.set_strength(self.hiddenids[j], self.urlids[k],
                                    1, self.wo[j][k])
        self.con.commit()
